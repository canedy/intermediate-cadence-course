...
- Well, you are borrowing the capability you created. At this point in the code, it is saying that we thought a private capability existed at `/private/NFTMinter,` but it never got created or given to the account or was unlinked. 
...
- Private storage is owned by the account, and people or things are given special access.

3. Write (in words) a scenario where you would use private capabilities. It cannot be the same NFT example we saw today

- Given that you are creating an Event and Ticketing system. Set up a contract that only the event admin can mint a new event and assign tickets. But once the event starts, the Admin might want to give temporary staff the ability to mint tickets and then revoke access after the event.
...
**Note**: In the future, I need to refactor this code so that minting the event and ticket resource happens in separate transactions. Also need to create a proxy in a way that it takes in some kind of role (Minter or Admin) to give the proper capability with less code. 
